...
/obj/machinery/computer/emergency_shuttle/examine(mob/user)
	...
	if(user?.mind?.get_hijack_speed())
		...
		. += span_danger("Alt click on this to attempt to hijack the shuttle. This will take multiple tries.")
...
/obj/machinery/computer/emergency_shuttle/proc/increase_hijack_stage()
	...
	return
	// NODALEC-REMOVE-SSshuttle.emergency - весь код ниже закомментирован
...
/obj/machinery/computer/emergency_shuttle/proc/attempt_hijack_stage(mob/living/user)
	...
	to_chat(user, span_boldwarning("You begin to override [src]'s navigational protocols."))
	...
	if(do_after(user, hijack_stage_time * (1 / user.mind.get_hijack_speed()), target = src))
		...
		message_admins("[ADMIN_LOOKUPFLW(user)] has hijacked [src] in [ADMIN_VERBOSEJMP(console_hijack_turf)].")
		user.log_message("has hijacked [src].", LOG_GAME)
		. = TRUE
